...
def getGeneOntology = {
  ...
  GDataConf goConf = new GDataConf(GFormat.RDF_XML, "data/pheno/go.owl")
...
def getGenes = {
  def genes = []
  def i = 0
  new File("data/pheno/mouse_annotations.tab").splitEachLine('\t') { items ->
    def s = 0
    if (!items[0].startsWith("#")) {
      genes.push(new Gene(i, new LinkedHashSet()))
      def gos = items[1].split("; ")
      for (int j = 0; j < gos.size(); j++) {
        genes[i].addAnnotation(getURIfromGO(gos[j]))
      }
...
def fout = new PrintWriter(new BufferedWriter(
  new FileWriter("data/pheno/go.txt")))
for (i = 0; i < result.size(); i++) {
  def x = i.intdiv(genes.size())
  def y = i % genes.size()
  if (x <= y) {
